...
public class IListView extends ListView implements IMessagerDispatcher, IMessageHandler {
    ...
    private OnScrollListener mProxyScrollListener = new OnScrollListener() {
                                                      ...
                                                      public void onScrollStateChanged(AbsListView view, int scrollState) {
                                                          ...
                                                          if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
                                                              ...
                                                              // 滚动到底部
                                                              if (!isPullUpLoading && isAutoPullUpEnabled()
                                                      ...
                                                      public void onScroll(AbsListView view, int firstVisibleItem,
                                                          ...
                                                          if (isSlideUp && oldScrollState != OnScrollListener.SCROLL_STATE_IDLE
                                                              && !isPullUpLoading && isAutoPullUpEnabled()
                                                          ...
                                                          if (null != subOnScrollListener) {
                                                              subOnScrollListener.onScroll(view, firstVisibleItem,
    ...
    private Runnable mRefreshFooterAndEmptyerRunnable = new Runnable() {
        ...
        @Override
        public void run() {
            mDataSetSize = IListView.this.getCount() - IListView.this.getHeaderViewsCount()
                           - IListView.this.getFooterViewsCount();
    ...
    public IListView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        pull_smooth_transition_duration = a.getInt(R.styleable.IListView_pull_smooth_transition_duration,
        ...
        pull_trigger_distance_pulldown = (int) a.getDimension(R.styleable.IListView_pull_trigger_dis_pulldown, (int) TypedValue
                .applyDimension(TypedValue.COMPLEX_UNIT_DIP, 15, getResources().getDisplayMetrics()));
        pull_trigger_distance_pullup = (int) a.getDimension(R.styleable.IListView_pull_trigger_dis_pullup, (int) TypedValue
                .applyDimension(TypedValue.COMPLEX_UNIT_DIP, 15, getResources().getDisplayMetrics()));
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mDataSetSize > 0) {// 只有在大于0的情况下才可以下拉/上拉动作, 否则只允许点击空白区域进行加载
                    ...
                    } else if (getLastVisiblePosition() == (mTotalItemCount - 1) && !isPullUpLoading && isPullUpEnabled()
                               && (deltaY < 0 || getFooterMargin() > 0)) {
            ...
            default:
                if (mDataSetSize > 0) {// 只有在大于0的情况下才可以下拉/上拉动作, 否则只允许点击空白区域进行加载
                    ...
                    } else if (getLastVisiblePosition() == (mTotalItemCount - 1) && !isPullUpLoading
                               && ((isPullUpEnabled() && canPullUp()))) {
    ...
    public void setEmptyViewClickable(boolean clickable) {
        if (mEmptyerView == null || !(mEmptyerView instanceof View)) {
            return;
        }

        ((View) mEmptyerView).setClickable(clickable);
    }
    ...
    public void stopPullLoading(int pullType, Runnable callBackAction) {
        if (pullType == STATUS_PULL_DOWN || pullType == STATUS_PULL_UP) {
            ...
            postDelayed(action,
                        elapse_time < pull_smooth_transition_duration ? pull_smooth_transition_duration - elapse_time : 1);
        }
    }
    ...
    private long stopPull(int pullType, int oldDataSetSize, int newListSize) {
        ...
        if (pullType == STATUS_PULL_DOWN && isPullDownLoading) {
            ...
            postDelayed(retsetHeaderRunnable, ms + 10);
            return ms;
            ...
            postDelayed(retsetFooterRunnable, ms + 10);
        }
    ...
    private class StopRunnable implements Runnable {
        ...
        public void run() {
            ...
            int newListSize = IListView.this.getCount() - IListView.this.getHeaderViewsCount()
                              - IListView.this.getFooterViewsCount();
